...
for i in "${!TARGET_NAME[@]}"; do
  ...
  if [[ -r "${N_DIR}" ]]; then
    if "${N_CRYPTOMATOR}"; then
      # check for macFUSE and FUSE-T mounts
      if ! mount | grep "^Cryptomator@macfuse[0-9]\+ on ${N_DIR} \|^localhost:[[:print:]]\+ on ${N_DIR} " > /dev/null 2>&1; then
        N_COLOR="${YELLOW}"
      fi
    fi
  else
...
check_and_execute()
{
  ...
  if ${SOURCE_IS_CRYPTOMATOR}; then
    # check for macFUSE and FUSE-T mounts
    if ! mount | grep "^Cryptomator@macfuse[0-9]\+ on ${SOURCE_DIR} \|^localhost:[[:print:]]\+ on ${SOURCE_DIR} " > /dev/null 2>&1; then
  ...
  for i in "${!TARGETS[@]}"; do
    ...
    if ${TARGET_IS_CRYPTOMATOR}; then
      # check for macFUSE and FUSE-T mounts
      if ! mount | grep "^Cryptomator@macfuse[0-9]\+ on ${TARGET_DIR} \|^localhost:[[:print:]]\+ on ${TARGET_DIR} " > /dev/null 2>&1; then
